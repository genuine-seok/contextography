...
const fonts: Font[] = [
  {
    name: "Abeezee",
    url: "http://themes.googleusercontent.com/static/fonts/abeezee/v1/JYPhMn-3Xw-JGuyB-fEdNA.ttf",
  },
  {
    name: "Lora",
...
const Home = () => {
  ...
  // 1. font 정보 동적 변경 적용 여부 확인하기
  // TODO: 커스텀 훅으로 분리
  // useEffect(() => {
  //   const fetchFont = async (font: Font) => {
  //     const newFont = new FontFace(`${font.name}`, `url(${font.url})`);
  //     document.fonts.add(newFont);
  //     // await document.fonts.ready;
  //     await newFont.load();
  //     if (mainRef.current) mainRef.current.style.fontFamily = `${font.name}`;
  //   };

  //   if (font) fetchFont(font);
  // });

  useEffect(() => {
    const fetchFontUrl = async (message: string) => {
